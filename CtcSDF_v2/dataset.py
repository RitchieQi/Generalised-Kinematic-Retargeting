...
class dexycb_base:
    def __init__(self, split, start_idx=None, end_idx=None):
        self.name = 'dexycb'
        self.data_split = split
        

class CtcSDF_dataset(Dataset):
    def __init__(self, start = None, end = None, data_dir = osp.join(osp.dirname(__file__), '..', 'CtcSDF', 'data'), task='train', sdf_sample=2048, sdf_scale=6.2, clamp=0.1, input_image_size=(256, 256)):
        self.start = start
        self.end = end
        self.task = task
        ...
        self.faces = torch.LongTensor(np.load(osp.join(osp.dirname(__file__), '..', 'CtcSDF','closed_fmano.npy')))
    ...
    def __getitem__(self, idx):
        ...
        # sample = self.getdata.get_sample(s0_id)
        ...
        # obj_transform = torch.vstack(torch.tensor(pose_y, dtype = torch.float32).squeeze(), torch.ones(1, 4))
        # obj_centre = torch.tensor(pose_y[:3,3], dtype = torch.float32).squeeze()
